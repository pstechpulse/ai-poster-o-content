...
def build_sota_video(has_ss, word_timings):
    ...
    # 1. DOWNLOAD EXPLICIT FONT (The Invisible Text Fix)
    print("📥 Downloading explicit font...")
    ...
    # 4. BOTTOM VISUAL (Assumes gameplay.mp4 is in your GitHub Repo!)
    if not os.path.exists("gameplay.mp4"):
        raise Exception("CRITICAL: You must upload 'gameplay.mp4' to your GitHub repository!")

    # 5. DYNAMIC SUBTITLE GENERATOR
    print("✍️ Generating text burned-in filters...")
    text_filters = []
    for item in word_timings:
        # Clean the word so it doesn't break FFmpeg syntax
        word = item['word'].replace("'", "").replace('"', '').upper()
        s, e = item['start'], item['end']
        # This explicitly uses the downloaded font.ttf
        text_filters.append(
    ...
    cmd = (
        f'ffmpeg -y {top_input} -i gameplay.mp4 -i voice.mp3 -i music.mp3 '
        f'-filter_complex "'
